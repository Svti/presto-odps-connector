...
public class OdpsTableLayoutHandle implements ConnectorTableLayoutHandle {
	...
	@JsonCreator
	public OdpsTableLayoutHandle(@JsonProperty("schemaTableName") SchemaTableName schemaTableName,
	...
	public OdpsTableLayoutHandle(SchemaTableName schemaTableName, List<OdpsColumnHandle> dataColumns,
			List<OdpsColumnHandle> desiredColumns, List<OdpsColumnHandle> partitionColumns,
			TupleDomain<ColumnHandle> partitionColumnPredicate, List<OdpsPartition> partitions) {
		this.schemaTableName = requireNonNull(schemaTableName, "table is null");
		this.dataColumns = ImmutableList.copyOf(requireNonNull(dataColumns, "dataColumns is null"));
		this.desiredColumns = ImmutableList.copyOf(requireNonNull(desiredColumns, "desiredColumns is null"));
		this.partitionColumns = ImmutableList.copyOf(requireNonNull(partitionColumns, "partitionColumns is null"));
	...
	public String toString() {
		return toStringHelper(this).add("schemaName", schemaTableName.getSchemaName())
				.add("tableName", schemaTableName.getTableName()).add("dataColumns", dataColumns)
				.add("partitionColumns", partitionColumns).toString();
	}
}
